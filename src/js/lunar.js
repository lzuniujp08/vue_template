...
const lunar = {
  ...
  lunarYearLength: function (y) {
    let sum = 12 * 29;
    for (let i = 0x8000; i > 0x8; i = i >> 1) {
      sum += (lunar.lunarInfo[y - 1900] & i ? 1 : 0);
    }
    sum += lunar.leapMonthLengths(y);
    return sum;
  },
  /**
   * 参数为农历日期
  ...
  year2GanZhe: function (y) {
    ...
    return lunar.Gan[gan - 1] + lunar.Zhe[zhe - 1];
  },
  /**
   * 参数为农历日期
  ...
  month2GanZhe: function (y, m) {
    ...
    let month = ((ganY - 1) % 5) * 12 + m + 1;
    return lunar.Gan[month % 10] + lunar.Zhe[month % 12];
  },
  /**
   * 参数为农历日期
  ...
  day2GanZhe: function (offset) {
    let temp = 40 + offset;
    return lunar.Gan[temp % 10] + lunar.Zhe[temp % 12];
  },
  /**
   * 参数均为公历日期
  ...
  sTerm: function (y, n) {
    ...
    let ms = 31556925974.7 * (y - 1900) + lunar.sTermInfo[n] * 60000;
    // 1900年一月六日两点五分是正小寒点，此点到1970年1.1 00：00：00的毫秒数
    let base = Date.UTC(1900, 0, 6, 2, 5);
    // 对应的公历日期
    let date = new Date(ms + base);
    return date;
  },
  /**
   * 参数为公历日期
  ...
  isTerm: function (y, m, d) {
    ...
    let n = (m - 1) * 2;
    let dateTerm = lunar.sTerm(y, n);
    if (lunar.isSameDay(dateTerm, date)) {
      return lunar.solarTerm[n];
    }
    n = n + 1;
    dateTerm = lunar.sTerm(y, n);
    if (lunar.isSameDay(dateTerm, date)) {
      return lunar.solarTerm[n];
    }
    return false;
  ...
  getStar: function (month, day) {
    let start = ['魔羯', '水瓶', '双鱼', '白羊', '金牛', '双子', '巨蟹', '狮子', '处女', '天秤', '天蝎', '射手', '魔羯'],
  ...
  toLunarMonth: function (m, flag) {
    if (flag) {
      return '闰' + lunar.lunarMonStr[m - 1] + '月';
    }
    return lunar.lunarMonStr[m - 1] + '月';
  },
  /**
   * 参数为农历日期
   * @method 根据日子获取中文写法
  ...
  toLunarDay: function (d) {
    ...
    return lunar.lunarStr[Math.floor(d / 10)] + (d % 10 === 0 ? '十' : lunar.weekend[d % 10]);
  },
  /**
   * 参数为公历年
   * @method 根据年份获取生肖
  ...
  isFestive: function (y, m, d, ly, lm, ld) {
    let lstr = lm > 9 ? (lm + '') : '0' + lm;
    lstr += ld > 9 ? (ld + '') : '0' + ld;
    let cstr = m > 9 ? (m + '') : '0' + m;
    cstr += d > 9 ? (d + '') : '0' + d;
    // 先算农历节日
    let festive = lunar.lFestive;
    ...
    // 算阳历
    festive = lunar.cFestive;
    for (var i = 0, l = festive.length; i < l; i += 1) {
      var test = festive[i].match(/(\d*#?\d*)-(.*)/);
      let year = test[1].split('#');
      if (year[1] && y > year[1]) {
        if (year[0] === cstr) {
          return test[2];
        }
    ...
    // 算星期
    festive = lunar.wFestive;
    let date = new Date(y, m - 1, d);
    for (var i = 0, l = festive.length; i < l; i += 1) {
      var test = festive[i].match(/(\d*)-(.*)/);
      let month = test[1].substring(0, 2);
      let day = test[1].substring(3, 4);
      let nWeek = test[1].substring(2, 3);
      let str = m > 9 ? (m + '') : '0' + m;
      if (str === month && date.getDay() === parseInt(day)) {
        let temp = lunar.theNoWeek(y, m, d);
  ...
  getlunarYMD: function (date) {
    ...
    let offset = (Date.UTC(result.year, result.month - 1, result.day) - Date.UTC(1900, 0, 31)) / (60 * 60 * 24 * 1000),
      temp = 0;
    for (var i = 1900; i < 2101 && offset > 0; i++) {
      temp = lunar.lunarYearLength(i);
      offset -= temp;
    }
    if (offset < 0) {
      offset += temp;
    ...
    let isLear = false,
      lunarMonth = lunar.leapMonth(result.lunarYear);
    for (var i = 1; i < 13 && offset >= 0; i++) {
      temp = lunar.lunarMonthLength(result.lunarYear, i);
      offset -= temp;
      isLear = false;
      if (i === lunarMonth) {
        if (offset < 0) {
          ...
          temp = lunar.leapMonthLengths(result.lunarYear);
          offset -= temp;
        }
      }
    }
    ...
    result.lunarMonth = i;
    result.lunarMonthChiness = lunar.toLunarMonth(result.lunarMonth, isLear);

    result.lunarDay = offset + 1;
    result.lunarDayChiness = lunar.toLunarDay(result.lunarDay);

    return result;
  },
  lunarTime: function (date) {
    let result = lunar.getlunarYMD(date);
    let festive = lunar.isFestive(result.year, result.month, result.day, result.lunarYear, result.lunarMonth, result.lunarDay);
    if (festive) {
      return festive;
    }
    let term = lunar.isTerm(result.year, result.month, result.day);
    if (term) {
      return term;
    }
  ...
  date2lunar: function (date) {
    ...
    let offset = (Date.UTC(result.year, result.month - 1, result.day) - Date.UTC(1900, 0, 31)) / (60 * 60 * 24 * 1000),
      temp = 0;
    // 干支记日
    result.gzD = lunar.day2GanZhe(offset);
    // 获取农历年
    for (var i = 1900; i < 2101 && offset > 0; i++) {
      temp = lunar.lunarYearLength(i);
      offset -= temp;
    ...
    result.lunarDayChiness = lunar.toLunarDay(result.lunarDay);
    // 获取生肖
    result.animal = lunar.getAnimal(result.year);
    // 获取星期
    result.week = '星期' + lunar.weekend[date.getDay()];
    // 获取星座
    result.start = lunar.getStar(result.month, result.day);
    // 获取干支年
    result.gzY = lunar.year2GanZhe(result.lunarYear);

    result.isTody = lunar.isTody(date);
    // 获取干支月
    result.gzM = lunar.month2GanZhe(result.lunarYear, result.lunarMonth);

    // 是否是节气，是返回节气名，不是返回false
    result.isTerm = lunar.isTerm(result.year, result.month, result.day);
    if (result.isTerm) {
      result.term = result.isTerm;
      result.isTerm = true;
    }
    // 是否是节日，是返回节日名，不是返回false
    result.isFestive = lunar.isFestive(result.year, result.month, result.day, result.lunarYear, result.lunarMonth, result.lunarDay);
    if (result.isFestive) {
